...
export default async function InboxPage() {
  ...
  return (
    <ScreenFrame
      ...
      banner={
        <DomainGap
          title="מודול ההודעות עדיין לא קיים — וזה מה שכן קיים במקומו"
          body={
            <>
              <p>
                אין במסד אף טבלה של הודעות, שיחות או משתתפים בשיחה. לכן אין כאן
                חוט שיחה, אין ״נקרא/לא נקרא״ ואין תשובה — ובמכוון גם אין מסך ריק
                שכתוב עליו ״אין שיחות פתוחות״: משפט כזה טוען שהמנגנון עובד
                ושפשוט לא הגיעו פניות, ושני חצאיו אינם נכונים.
              </p>
              <p className="mt-2">
                מה שמוצג למטה הוא שלוש רשימות של רשומות אמיתיות, כל אחת בשמה:
                מילים של האורח ששמורות על ההזמנה, בקשת אורח שהפכה למשימה, והערה
                פנימית שהצוות השאיר לעצמו. רק השנייה ניתנת להקצאה — כי{' '}
                <span dir="ltr" className="font-mono text-xs">
                  task_assignments
                </span>{' '}
                קיימת ו־<GrantCode>message.assign</GrantCode> אין לה למה לכתוב.
              </p>
            </>
          }
