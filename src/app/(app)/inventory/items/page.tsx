...
export const metadata: Metadata = { title: 'פריטי מלאי' }

/**
 * EXECUTION CONTEXT — SERVER COMPONENT. Stock by item and location.
 *
 * WHERE THIS CAME FROM. This screen was `/inventory` and is now
 * `/inventory/items`, moved rather than rewritten: the columns, the redactions,
 * the reorder-point banner, the movement ledger and every empty state are the
 * ones that already shipped, and the only edits are the route it links back to
 * and the module gate below. `/inventory` is now the dashboard, because nobody
 * opens a stock module to read forty rows of quantities — they open it because
 * they are worried about Saturday.
 *
 * THE MODULE GATE. An organization with `mode = off` never sees a stock widget,
 * this list included. That is checked before anything renders and not after,
 * because a summary tile drawn "harmlessly" above an empty table is exactly how
    ...
    <div className="mx-auto flex w-full max-w-shell flex-col gap-6 px-4 py-6 sm:px-6 sm:py-10 lg:px-8">
      ...
      {failure ? (
        ...
        <>
          ...
          {/*
            The way into one item's own screen — its ledger, its reservations
            and its projection. A separate list rather than a link inside the
            table, because `InventoryTable` is shared with other screens and a
            column added for this one would appear on all of them.
          */}
          <nav aria-label="מעבר לפריט" className="flex flex-wrap gap-2">
            {items.map((item) => (
              <Button
                key={item.id}
